...
class GoldenRetrieverPredictionCallback(PredictionCallback):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # with open(f"predictions_{trainer.global_rank}.json", "w") as f:
        #     json.dump(dataloader_predictions, f, indent=2)

        # return the predictions
        return dataloader_predictions


class GoldenRetrieverFabricPredictionCallback(PredictionFabricCallback):
    def __init__(
        self,
        k: int | None = None,
        batch_size: int = 32,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        self.datasets = [d.dataset for d in dataloaders]
        ...
        for dataloader_idx, dataloader in enumerate(self.dataloaders):
            ...
            # check if we need to reindex the passages and
            # also if we need to load the retriever from disk
            # if (self.retriever_dir is not None and current_epoch == 0) or (
            #     self.retriever_dir is not None and stage == RunningStage.TESTING
            # ):
            #     force_reindex = False
            # else:
