...
class RecallAtKEvaluationCallback(NLPTemplateCallback):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.verbose:
            log.info(f"metrics/{dataloader_label}/recall@{self.k}: {recall_at_k}")
...
class AvgRankingEvaluationCallback(NLPTemplateCallback):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        dataloader_label = state.dataloader_label
        logger.log_metrics(
            {f"metrics/{dataloader_label}/avg_ranking@{self.k}": avg_ranking}
        )
        state.eval_metrics[f"metrics/{dataloader_label}/avg_ranking@{self.k}"] = (
            avg_ranking
        )
        state.eval_metrics[f"metrics/avg_ranking@{self.k}"] = avg_ranking
        if self.verbose:
            log.info(f"metrics/{dataloader_label}/avg_ranking@{self.k}: {avg_ranking}")
