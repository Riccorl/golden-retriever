...
def preprocess(
    ...
    with open(output_file_path, "w") as f:
        for doc_id, document in tqdm(enumerate(data), desc="Windowizing documents"):
            ...
            num_entities = (
                len(document["entities"]) + len(document["entities_wsd"])
                if use_wsd
                else len(document["entities"])
            )
            if num_entities == 0:
            ...
            doc_level_labels = (
                document["entities"] + [[d[0], d[1], d[2]] for d in document["entities_wsd"]]
                if use_wsd
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--use_wsd", action="store_true")
